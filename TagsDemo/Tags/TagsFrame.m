...
- (void)setTagsArray:(NSArray *)tagsArray{
    ...
    for (NSInteger i = 0; i < lastIndexs.count; i++) {
        ...
        //从数组中截取每一行的数组
        NSArray *newArray = [tagsArray subarrayWithRange:NSMakeRange(location, length)];
        location = lastIndex + 1;
        
        averageW = [moreWidths[i] floatValue]/newArray.count;
        
        CGFloat tagX = _tagsMargin;
        CGFloat tagY = _tagsLineSpacing + (_tagsLineSpacing + tagH) * i;
        for (NSInteger j = 0; j<newArray.count; j++) {
            tagW = [self sizeWithText:newArray[j] font:TagsTitleFont].width + _tagsMinPadding * 2 + averageW;
